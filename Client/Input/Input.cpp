...
bool Input::Init()
{
	_hInst = GAME_ENGINE.GetHINSTANCE();
	_hWnd = GAME_ENGINE.GetHWND();
	...
	if (eInputSystemType::DINPUT == _type)
	{
		if (false == InitInput())
		{
			return false;
...
bool Input::InitInput()
{
	...
	{
		if (FAILED(_keyboard->SetCooperativeLevel(_hWnd, DISCL_BACKGROUND | DISCL_NONEXCLUSIVE)))
		{
			return false;
		}
	}
	...
	{
		if (FAILED(_keyboard->SetCooperativeLevel(_hWnd, DISCL_BACKGROUND | DISCL_EXCLUSIVE)))
		{
			return false;
		}
	}
...
bool Input::UpdateKeyBoard()
{
	...
	if (FAILED(ret))
	{
		if (DIERR_INPUTLOST == ret || DIERR_NOTACQUIRED == ret)
		{
			_keyboard->Acquire();
		}
	}
...
bool Input::UpdateMouse()
{
	...
	HRESULT ret = _mouse->GetDeviceState(sizeof(_mouse), (LPVOID)&_mouseState);
	if (FAILED(ret))
	{
		if (DIERR_INPUTLOST == ret || DIERR_NOTACQUIRED == ret)
		{
...
void Input::UpdateInput(float deltaTime)
{
	if (_type == eInputSystemType::DINPUT)
	{
		for (int i = 0; i < MOUSE_BUTTON_TYPE::eType::END; i++)
		{
...
void Input::UpdateMousePos(float deltaTime)
{
	POINT mousePT;
